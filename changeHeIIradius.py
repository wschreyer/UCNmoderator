...
for line in fileinput.input('ucn.inp', inplace = 1):
  ...
  if match:
    m = list(match.groups())
    if m[1] in ['ld2o', 'ld2obott', 'ld2bottl', 'ld2', 'ld2i', 'ld2ibott', 'heiibott', 'heii', 'isovac', 'vactank']:
      m[8] = '{0:g}'.format(float(m[8]) + inc) # increase radius
      for i in m:
        sys.stdout.write(i+' ')
      sys.stdout.write('\n')
    else:
...
for line in fileinput.input('ucn.mcnp', inplace = 1):
  ...
  if match:
    m = list(match.groups())
    if m[0] in ['19', '20', '22', '23', '24', '25', '35', '36', '37', '38']:
